...
rl.question("apiコンテナを作成しますか？(y/n)", async answer => {
  ...
  // ファイルの指定行番号に追記する
  const data = fs
    .readFileSync(`${motherDir}/${innerDockerComposeFile}`)
    .toString()
    .split("\n");
  data.splice(2, 0, apiService);
  const text = data.join("\n");
  await fs.writeFile(`${motherDir}/${innerDockerComposeFile}`, text, function(
    err
  ) {
    if (err) throw err;
  });
  ...
  // Dockerfile用意
  await fs.copyFile(
    "./resources/api_Dockerfile",
    `${motherDir}/api/Dockerfile`,
    err => {
      if (err) throw err;
  ...
  // server.jsを用意
  await fs.copyFile(
    "./resources/api_server.js",
    `${motherDir}/api/server.js`,
    err => {
      if (err) throw err;
  ...
  // package.jsonを用意
  await fs.copyFile(
    "./resources/api_package.json",
    `${motherDir}/api/package.json`,
    err => {
      if (err) throw err;
